...
(function() {
    ...
    function createPreferencesBox(selector) {
        ...
        if (!prefBox) {
            ...
            box.innerHTML = `<div class="hardy_modal"><div class="hardy_modal_header"><label>Preferences</label></div><div style="text-align: right;"><button class="hardy_modal_close">Close</button></div><div style="overflow: auto;"><div class="hardy_modal_content"></div><div class="hardy_modal_msg"></div></div></div>`;
            ...
            box.querySelector(".hardy-save-prefs").addEventListener("click", ()=> {
                ...
                box.querySelector(".hardy_modal_msg").innerHTML = `Preferences have been saved successfully!!`;
            });
        }
    }
        ...
        switch (inpType) {
            ...
            case "select":
                if (saved.select[name]) {
                    return saved.select[name] !== "N/A" && saved.select[name] === opt ? " selected" : "";
                } else {
                    saved.select[name] = def;
                    return opt === def ? " selected" : "";
                }
        }
    }
